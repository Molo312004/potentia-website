...
async function checkIfTestAlreadyTaken() {
    ...
    try {
        const response = await fetch(`${CONFIG.API_BASE_URL}/api/test-results/by-email/${userData.email}/test/Chemistry`, {
            method: "GET",
            headers: {
...
function showAlreadyTakenMessage(testResult) {
    ...
    mainContent.innerHTML = `
        <div style="text-align: center; padding: 40px; background: #f0f4f0; border-radius: 10px; margin: 20px;">
            <h2 style="color: #8B4513; margin-bottom: 20px;">You've Already Taken This Test</h2>
            ...
            <div style="background: white; padding: 30px; border-radius: 10px; display: inline-block; margin-bottom: 30px;">
                <h3 style="color: #8B4513; margin-bottom: 20px;">Your Score</h3>
                <div style="margin: 15px 0;">
                    <p><strong>Total Marks:</strong> <span style="color: #8B4513; font-size: 20px;">${testResult.totalMarks}/80</span></p>
                    ...
                    <p><strong>Accuracy:</strong> <span style="color: #8B4513;">${((testResult.correctAnswers / 20) * 100).toFixed(2)}%</span></p>
                    <p><strong>Submitted:</strong> ${new Date(testResult.submittedAt).toLocaleString()}</p>
                </div>
            </div>
            
            <a href="scholarship.html" style="display: inline-block; padding: 12px 30px; background-color: #8B4513; color: white; text-decoration: none; border-radius: 5px; font-size: 16px; margin-top: 20px;">Back to Scholarship Page</a>
        </div>
    `;
}
...
function loadQuestion(index) {
    if (index < 0 || index >= chemistryQuestions.length) return;
    
    currentQuestion = index;
    const question = chemistryQuestions[index];
    const questionSection = document.getElementById("questionSection");
    
    // Update progress bar
    const progress = ((index + 1) / chemistryQuestions.length) * 100;
    ...
    document.getElementById("nextBtn").disabled = index === chemistryQuestions.length - 1;
    document.getElementById("nextBtn").textContent = index === chemistryQuestions.length - 1 ? "Last Question" : "Next →";
}
...
function calculateMarks() {
    ...
    chemistryQuestions.forEach((question, idx) => {
...
function showResultModal(marks) {
    document.getElementById("totalQuestionsResult").textContent = "20";
...
// Close modal and go back to scholarship page
